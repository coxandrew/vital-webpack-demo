// Framework Icons

$icon-arrow-updown: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50"><path d="M11.4 20.8h26.8L24.8 0 11.4 20.8m26.8 8.4H11.4L24.8 50l13.4-20.8"/></svg>';

$icon-check: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50"><path d="M44.3 12.2l-2.5-2.6-24 24.3-9.1-9.1-2.5 2.5 10.4 10.4c.4.4.9.7 1.3.7.3 0 .8-.3 1.2-.7l25.2-25.5z"/></svg>';

$icon-check-inverse: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50"><style type="text/css">.st0{fill:#fff;}</style><path class="st0" d="M44.3 12.2l-2.5-2.6-24 24.3-9.1-9.1-2.5 2.5 10.4 10.4c.4.4.9.7 1.3.7.3 0 .8-.3 1.2-.7l25.2-25.5z"/></svg>';

$icon-clock: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50"><path d="M25 0C11.2 0 0 11.2 0 25s11.2 25 25 25 25-11.2 25-25S38.8 0 25 0zm0 47.1C12.8 47.1 2.9 37.2 2.9 25S12.8 2.9 25 2.9 47.1 12.8 47.1 25 37.2 47.1 25 47.1z"/><path d="M26.5 21.9V6.4c0-.9-.6-1.5-1.5-1.5s-1.5.6-1.5 1.5v15.5c-1.2.5-2 1.7-2 3.1 0 .4.1.8.2 1.2l-5.6 5.6c-.6.6-.6 1.5 0 2.1.3.3.8.4 1 .4.3 0 .7-.1 1-.4l5.6-5.6c.4.1.7.2 1.2.2 1.9 0 3.4-1.5 3.4-3.4.1-1.5-.6-2.6-1.8-3.2z"/></svg>';

$icon-close: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50"><path d="M42.7 7.3C33-2.5 17.1-2.5 7.4 7.3s-9.7 25.6 0 35.3 25.6 9.8 35.3 0 9.7-25.5 0-35.3zm-6.1 27.8l-1.4 1.4L25 26.4 14.9 36.5l-1.4-1.4L23.6 25 13.5 14.9l1.4-1.4L25 23.6l10.1-10.1 1.4 1.4L26.4 25l10.2 10.1z"/></svg>';

$icon-close-empty: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50"><path d="M44.4 7.7l-2.1-2.1L25 22.9 7.7 5.6 5.6 7.7 22.9 25 5.6 42.3l2.1 2.1L25 27.1l17.3 17.3 2.1-2.1L27.1 25 44.4 7.7z"/></svg>';

$icon-close-empty-white: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50"><style>.st0{fill: white}</style><path class="st0" d="M44.4 7.7l-2.1-2.1L25 22.9 7.7 5.6 5.6 7.7 22.9 25 5.6 42.3l2.1 2.1L25 27.1l17.3 17.3 2.1-2.1L27.1 25 44.4 7.7z"/></svg>';

$icon-close-outline: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50"><path d="M42.7 7.3C33-2.5 17.1-2.5 7.4 7.3s-9.8 25.6 0 35.4c9.8 9.8 25.6 9.8 35.4 0s9.7-25.6-.1-35.4zm-1.4 33.9c-8.9 8.9-23.5 8.9-32.4 0S0 17.7 8.9 8.8s23.5-8.9 32.4 0 8.9 23.5 0 32.4z"/><path d="M35.2 13.4L25 23.6 14.9 13.5l-1.4 1.4L23.6 25 13.5 35.1l1.4 1.4L25 26.4l10.1 10.1 1.4-1.4L26.4 25l10.1-10.1-1.3-1.5z"/></svg>';

$icon-menu: 'data:image/svg+xml;utf8,<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="512px" height="512px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve"><path d="M0,91.2v20.5h512V91.2H0z M0,244.8v20.5h512v-20.5H0z M0,398.4v20.5h512v-20.5H0z"/></svg>';

$icon-menu-white: 'data:image/svg+xml;utf8,<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="512px" height="512px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve"><style>.white{fill: white}</style><path class="white" d="M0,91.2v20.5h512V91.2H0z M0,244.8v20.5h512v-20.5H0z M0,398.4v20.5h512v-20.5H0z"/></svg>';
